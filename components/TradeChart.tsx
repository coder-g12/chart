...
import TradeChartToolbar from "./TradeChartToolbar";
...
const TradeChart: React.FC = () => {
    ...
    const chartInstanceRef = useRef<{
        ...
        timeToTopBottom?: Map<number, string>;
    ...
    const computeColorForType = (tb: string | undefined, mirrorActive: boolean, seriesType: "IM" | "MM") => {
        ...
        if (mirrorActive) {
            if (baseColor === (seriesType === "IM" ? IM_GREEN : MM_GREEN)) return seriesType === "IM" ? IM_RED : MM_RED;
            if (baseColor === (seriesType === "IM" ? IM_RED : MM_RED)) return seriesType === "IM" ? IM_GREEN : MM_GREEN;
        }
        return baseColor;
    };
    ...
    useEffect(() => {
        ...
        const chart = createChart(priceRef.current, {
            ...
            crosshair: { mode: 1 },
            rightPriceScale: { borderColor: "#555555", scaleMargins: { top: 0.05, bottom: 0.1 } },
            timeScale: {
                borderColor: "#555555",
                timeVisible: true,
        ...
        const validData = data.filter(d => d.time && typeof d.time === "string" && d.time.includes(" "));
        const timeToTopBottom = new Map<number, string>();
        validData.forEach(d => timeToTopBottom.set(getTime(d), d.topbottom ?? ""));

        const imSeries = chart.addHistogramSeries({ color: DEFAULT_HIST_COLOR, priceScaleId: "im-scale", visible: false });
        const mmSeries = chart.addHistogramSeries({ color: DEFAULT_HIST_COLOR, priceScaleId: "mm-scale", visible: false });
        ...
        chart.priceScale("im-scale").applyOptions({ scaleMargins: { top: 0.6, bottom: 0.05 }, borderColor: "#555555" });
        chart.priceScale("mm-scale").applyOptions({ scaleMargins: { top: 0.6, bottom: 0.05 }, borderColor: "#555555" });

        const lastIM = histDataIM[histDataIM.length - 1];
        const lastMM = histDataMM[histDataMM.length - 1];
        const imPriceLine = imSeries.createPriceLine({ price: lastIM.value, color: 'white', lineWidth: 1, lineStyle: 0, axisLabelVisible: true, title: 'IM' });
        const mmPriceLine = mmSeries.createPriceLine({ price: lastMM.value, color: 'white', lineWidth: 1, lineStyle: 0, axisLabelVisible: true, title: 'MM' });
        ...
        const updateStrSeries = () => {
            ...
            for (let i = 0; i < validData.length; i++) {
                ...
                if (time >= visibleRange.from && time <= visibleRange.to && (validData[i].topbottom === "T" || validData[i].topbottom === "B")) {
                    anchorIndex = i;
                    break;
                }
            }
            ...
            for (let i = 0; i < validData.length; i++) {
                ...
                if (t >= visibleRange.from && t <= visibleRange.to) {
                    maxPrice = Math.max(maxPrice, validData[i].high ?? validData[i].close);
                }
            }
            ...
            const displayedAnchorPrice = mirrorActive ? -anchorOriginalPrice + maxPrice : anchorOriginalPrice;
            ...
            strSeries.forEach((series, key) => {
                ...
                if (!original) return;
                const strValAtAnchor = (validData[anchorIndex][strType as keyof CandleData] as number[])[i];
                const offset = (mirrorActive ? -strValAtAnchor : strValAtAnchor) - displayedAnchorPrice;
                const transformed = original.map(pt => ({ time: pt.time, value: (mirrorActive ? -pt.value : pt.value) - offset }));
        ...
        const updateMirror = () => {
            const inst = chartInstanceRef.current;
            if (!inst) return;
            const { chart, priceSeries, strSeries, originalStrData, originalPriceData, originalHistIM, originalHistMM, imSeries: imS, mmSeries: mmS, timeToTopBottom } = inst;
            if (!originalPriceData || !priceSeries) return;
            const visibleRange = chart.timeScale().getVisibleRange();
            if (!visibleRange) return;

            const inViewport = (t: Time) => t >= visibleRange.from && t <= visibleRange.to;
            let maxPrice = -Infinity;
            for (let i = 0; i < validData.length; i++) {
                const t = getTime(validData[i]);
                if (inViewport(t)) maxPrice = Math.max(maxPrice, validData[i].high ?? validData[i].close);
            }
            maxPrice = maxPrice === -Infinity ? 0 : maxPrice;
            ...
            const transformedPrice = originalPriceData.map(p => {
                if ("value" in p) {
                    ...
                    return inViewport(pt.time as Time) ? { time: pt.time, value: -pt.value + maxPrice } : pt;
                } else {
            ...
            if (showTB && "setMarkers" in priceSeries) {
                ...
                validData.forEach(d => {
                    ...
                    if (isT) markers.push({ time, position: 'inBar', color: GREEN, shape: 'circle', size: 1 });
                    if (isB) markers.push({ time, position: 'inBar', color: RED, shape: 'circle', size: 1 });
                });
            ...
            strSeries.forEach((series, key) => {
                ...
                if (!orig) return;
                const mirrored = orig.map(pt => ({ time: pt.time, value: -pt.value }));
                series.setData(mirrored);
            });

            let maxIM = -Infinity, maxMM = -Infinity;
            for (let i = 0; i < validData.length; i++) {
                const t = getTime(validData[i]);
                if (inViewport(t)) {
            ...
            if (originalHistIM && imS) {
                const newIM = originalHistIM.map(h => inViewport(h.time as Time) ? {
                    time: h.time,
                    value: maxIM - (h.value as number),
                    color: computeColorForType(timeToTopBottom?.get(h.time as number), true, "IM")
                } : h);
                imS.setData(newIM);
            ...
            if (originalHistMM && mmS) {
                const newMM = originalHistMM.map(h => inViewport(h.time as Time) ? {
                    time: h.time,
                    value: maxMM - (h.value as number),
                    color: computeColorForType(timeToTopBottom?.get(h.time as number), true, "MM")
                } : h);
                mmS.setData(newMM);
        ...
        chartInstanceRef.current = {
            chart, imSeries, mmSeries, strSeries: new Map(), originalStrData: new Map(),
            originalHistIM: histDataIM, originalHistMM: histDataMM, timeToTopBottom,
            updateStrSeries, updateMirror, mirrorActive: false, imPriceLine, mmPriceLine
        };
    ...
    useEffect(() => {
        ...
        if (priceType === "candle") {
            newSeries = chart.addCandlestickSeries({
                upColor: "#00ff00", downColor: "#ff0000", borderUpColor: "#00ff00",
                borderDownColor: "#ff0000", wickUpColor: "#00ff00", wickDownColor: "#ff0000",
            });
            const candleData: CandlestickData[] = validData.map(d => ({
                time: getTime(d), open: d.open, high: d.high, low: d.low, close: d.close,
            }));
            ...
            const lineData: LineData[] = validData.map(d => ({ time: getTime(d), value: d.close }));
            ...
            if (showTB) {
                ...
                validData.forEach(d => {
                    const time = getTime(d);
                    const tb = (d.topbottom ?? "").toString().toUpperCase();
                    let isT = tb === "T", isB = tb === "B";
                    if (mirrorOn) [isT, isB] = [isB, isT];
                    if (isT) markers.push({ time, position: 'inBar', color: GREEN, shape: 'circle', size: 1 });
                    if (isB) markers.push({ time, position: 'inBar', color: RED, shape: 'circle', size: 1 });
                });
                try { (newSeries as any).setMarkers(markers); } catch {}
            }
            chartInstanceRef.current!.originalPriceData = lineData;
        }
        chartInstanceRef.current!.priceSeries = newSeries;
        if (mirrorOn && chartInstanceRef.current.updateMirror) chartInstanceRef.current.updateMirror();
    ...
    useEffect(() => {
        ...
        const validData = data.filter(d => d.time && typeof d.time === "string" && d.time.includes(" "));
        const strTypes: (keyof CandleData)[] = ["drive", "harmony", "root", "action", "expand", "live"];
        ...
        strTypes.forEach((strType, typeIndex) => {
            ...
            strArray.forEach((_, i) => {
                ...
                if (showStrs[strType][i] && !strSeries.has(key)) {
                    const series = chart.addLineSeries({ color: colors[(typeIndex * 5 + i) % colors.length] });
                    const lineData: LineData[] = validData.map(d => ({ time: getTime(d), value: (d[strType] as number[])[i] }));
    ...
    useEffect(() => {
        ...
        chart.priceScale("right").applyOptions({
            scaleMargins: { top: 0.05, bottom: showIM && showMM ? 0.6 : showIM || showMM ? 0.35 : 0.1 },
        });
        if (imSeries) {
            imSeries.applyOptions({ visible: showIM });
            if (showIM) chart.priceScale("im-scale").applyOptions({
                scaleMargins: { top: 0.6, bottom: showMM ? 0.2 : 0.05 }, borderColor: "#555555"
            });
        }
        if (mmSeries) {
            mmSeries.applyOptions({ visible: showMM });
            if (showMM) chart.priceScale("mm-scale").applyOptions({
                scaleMargins: { top: showIM ? 0.8 : 0.6, bottom: 0.0 }, borderColor: "#555555"
            });
        }
    }, [showIM, showMM]);
    ...
    useEffect(() => {
        ...
        if (mirrorOn) {
            ...
            if (updateMirror) chart.timeScale().unsubscribeVisibleTimeRangeChange(updateMirror);
            if (inst.priceSeries && inst.originalPriceData) inst.priceSeries.setData(inst.originalPriceData as any);
            inst.strSeries.forEach((series, key) => {
            ...
            if (showTB && inst.priceSeries) {
                ...
                validData.forEach(d => {
                    const time = getTime(d);
                    const tb = (d.topbottom ?? "").toString().toUpperCase();
                    const isT = tb === "T", isB = tb === "B";
                    if (isT) markers.push({ time, position: 'inBar', color: GREEN, shape: 'circle', size: 1 });
                    if (isB) markers.push({ time, position: 'inBar', color: RED, shape: 'circle', size: 1 });
                });
                try { (inst.priceSeries as any).setMarkers(markers); } catch {}
            }
        }
    }, [mirrorOn]);
